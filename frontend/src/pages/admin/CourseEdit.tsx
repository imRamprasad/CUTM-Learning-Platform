...
import { CourseLevel, CourseStatus, getCourseById, updateCourse } from '../../data/adminApi';
...
export default function AdminCourseEdit() {
  ...
  useEffect(() => {
    ...
    setVideos(course.videos.length ? course.videos : ['']);
    setArticles(course.articles.length ? course.articles : ['']);
    setLoaded(true);
  }, [courseId]);
  ...
  if (loaded && !getCourseById(courseId)) {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
        <h1 className="text-xl font-bold text-slate-900">Course not found</h1>
        <p className="text-sm text-slate-500 mt-2">This course record does not exist.</p>
        <Link to="/admin/courses" className="inline-flex mt-4 text-indigo-600 hover:text-indigo-700 text-sm font-medium">
          Back to course list
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Update content and publication details.</p>
      </div>
      ...
      <form onSubmit={onSubmit} className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label htmlFor="course-status" className="block text-sm font-medium text-slate-700 mb-1">Status</label>
            <select
              id="course-status"
              value={status}
